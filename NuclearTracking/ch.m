...
% function [U,mov]=ch(zi3)

function U=ch(zi3,epsilon)

N = size(zi3,2);  M = size(zi3,1);
delx = 1/(M-1);
delx2 = delx^2;
x = (0:delx:1)';
...
if(~exist('epsilon','var'))
    epsilon = 0.05;   % sets characteristic length: at 0.0002 and above nuclei bleed together
end
eps2 = epsilon^2;
...
% files=dir;
inmat=zi3;
U=(0.0001*(double(inmat)-0.5*double(mean(inmat(:)))));
% U = (rand(N,M)-0.5)*0.01;
hat_U = dct2(U);
...
% plotting and movie
  ...
  if rem(it,visual_update) == 0
    ...
    subplot(2,2,2)
    mesh(U), axis([0 N 0 M -.02 .04]), title('U');
    vhat = abs(hat_U); vhat(1,1)=0; 
    vhat = vhat/max(max(abs(vhat))); 
    subplot(2,2,3)
    plot(x',U(ceil(M/2),:)), axis([0 1 -.01 .03]), ...
