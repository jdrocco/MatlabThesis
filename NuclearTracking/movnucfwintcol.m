...
h=imagesc(imfine); colormap(gray(256));hold on;
    ...
    set(gca,'ytick',0);
    for m=1:size(ytrace,1)
    line(round([yfine(m,i+offset) yfine(m,i+offset)+5]),round([xfine(m,i+offset) xfine(m,i+offset)-6]),'color',squeeze(map((colmult*mod(colind(m),(round(56/colmult)+1)))+1,:)),'LineWidth',2)
    end
