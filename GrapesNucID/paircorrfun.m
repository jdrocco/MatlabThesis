function [pairycory,xmpp]=paircorrfun(DNApaircent,PHIScent,xmlfilename)

tree=xml_read(xmlfilename);
xmlstruct=leicaxmlextract(tree);
xmpp=1000000*xmlstruct.xdimreal/xmlstruct.xpixels;
ympp=1000000*xmlstruct.ydimreal/xmlstruct.ypixels;
if((abs(xmpp-ympp))>0.01)
    disp('Nonsquare pixels!!');
end
...
figure; hold on;
for i=1:250
    plot(i*xmpp,mean(pairycory(round(pairycory(:,1))==i,2))-(((size(DNApaircent,1)-size(PHIScent,1))/(size(DNApaircent,1)+size(PHIScent,1)))^2),'+');
end
xlabel('|r_i-r_j|'); ylabel('<\pi_i\pi_j> - <\pi_i><\pi_j>');

figure; hold on;
for i=1:250
    plot(i*xmpp,mean(pairycory(round(pairycory(:,1))==i,3)),'+');
end
