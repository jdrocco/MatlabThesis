...
% Within crestprep tree
% be careful to keep imchan1 and imchan2 straight
% Deprecated with use of NLS to smooth background

function [images,time,zstep,header,createdpts,crapims]=crestims(smoothbgd,number_of_frames,stacksize,pathname,filename,twochaninput,xdimreal,ydimreal)
...
crossimfactor=0; % Factor by which to multiply channel 2 before subtraction
        ...
        for k=1:linpixelsx
            for l=1:linpixelsy
                ...
                elseif(((i+1)<(number_of_frames+1))&&(k>relpxpos(i+1,i))&&(k<(relpxpos(i+1,i)+linpixelsx+1))&&(l>relpypos(i+1,i))&&(l<(relpypos(i+1,i)+linpixelsy+1)))
                    ...
                    occ(j,k+srelpxpos(i),(l+srelpypos(i)))=occ(j,k+srelpxpos(i),l+srelpypos(i))+smoothbgd(k,l);
                end
            end
        end
    end
...
images=int16((mastnotim-crossimfactor*(mastim-consttosubtr))./occ);
crapims=int16(mastim./occ);
