function [optdiffmat,summat]=fitcompexp(gomcompavg,egglcomp,timecomp,avscomp,depth,zsize,maxevals,bgdlvl,trydiffc)
...
%     gomrsout=realspacegom(gomcompavg(i,:,:),egglcomp(i));
    for firstidx=1:(numtimes-1)
        [optdiffc,bestsqsum,exitflag,output]=fitwbndsexp(egglcomp(i),firstidx,squeeze(gomcompavg(i,:,:)),timecomp(i,:),squeeze(avscomp(i,:,:)),depth,zsize,trydiffc,maxevals,bgdlvl);
