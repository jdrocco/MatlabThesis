...
function sortlifeser(stagesorer,eggnum,perfol,egglookup)
...
%             copyfile(strcat('Pos',sprintf('%03d',a),'*t',sprintf('%d',b-1),'*tif'),folname)
    ...
    for j=1:length(egginds)
        ...
        movefile(strcat('Series',sprintf('%03d',lookeduppos),'*t',sprintf('%03d',b-1),'*tif'),folname)
        copyfile(strcat('Series',sprintf('%03d',lookeduppos),'*xml'),folname)
    end
end